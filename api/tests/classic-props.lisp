...
(5am:test (prop-sum-to)
    (5am:for-all ((hi (5am:gen-integer :min 0 :max 18))
		...
		(5am:is (and (reduce (lambda (a f)
			...
			(reduce (lambda (a f)
			(and a (= ans (lazy:head (lazy-seqs:drop (- hi lo)
				(funcall f lo))))))
...
(5am:test (prop-fact)
    (5am:for-all ((n (5am:gen-integer :min 0 :max 18)))
		...
		(5am:is (and (reduce (lambda (a f)
			...
			:initial-value t)
			(reduce (lambda (a f)
			(and a (= ans (lazy:head (lazy-seqs:drop n (funcall f))))))
...
(5am:test (prop-pascaltri)
    (5am:for-all ((rows (5am:gen-integer :min 0 :max 10)))
		...
		(5am:is (and (reduce (lambda (a f)
			...
			:initial-value t)
			(reduce (lambda (a f)
			(and a (equal ans (lazy-seqs:take (1+ rows) (funcall f)))))
