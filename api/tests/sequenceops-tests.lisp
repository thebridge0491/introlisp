...
(5am:test (test-index-find :fixture fixc-sequenceops)
	(mapcar (lambda (tup)
		...
		'((seqops:index-r . seqops:find-r) (seqops:index-i . seqops:find-i)
...
(5am:test (test-min-max :fixture fixc-sequenceops)
	(mapcar (lambda (tup)
		...
		(seqops:min-lp . seqops:max-lp) (seqops:min-f . seqops:max-f)
...
(5am:test (test-any-every-v :fixture fixc-sequenceops)
	(let ((xs '(1 2)) (ys '(3)) (zs '(4 5))
		(pred-any2 (lambda (e1 e2) (or (null e1) (null e2))))
		(pred-any3 (lambda (e1 e2 e3) (or (null e1) (null e2) (null e3))))
		(pred-every2 (lambda (e1 e2) (and (consp e1) (consp e2))))
		(pred-every3 (lambda (e1 e2 e3)
			(and (consp e1) (consp e2) (consp e3)))))
	(mapcar (lambda (tup)
		(let* ((fn-any (car tup)) (fn-every (cdr tup)))
			(5am:is (equal (some pred-any2 xs '(6 ())) 
				(funcall fn-any pred-any2 xs '(6 ()))))
			(5am:is (equal (some pred-any3 xs zs '(6 ())) 
				(funcall fn-any pred-any3 xs zs '(6 ()))))
			(5am:is (equal (every pred-every2 (list xs) (list zs)) 
				(funcall fn-every pred-every2 (list xs) (list zs))))
			(5am:is (equal (every pred-every3 (list xs) (list ys) (list zs))
				(funcall fn-every pred-every3 (list xs) (list ys) (list zs))))))
		'((seqops:any-rv . seqops:every-rv)
			(seqops:any-iv . seqops:every-iv)
			(seqops:any-lpv . seqops:every-lpv)
			(seqops:any-fv . seqops:every-fv)
			(seqops:any-uv . seqops:every-uv)))))

(5am:test (test-map-v :fixture fixc-sequenceops)
	(let* ((xs '(0 1)) (ys '(2 3)) (zs '(4 5))
		(proc2 (lambda (a b) (list (+ a 2) (+ b 2))))
		(proc3 (lambda (a b c) (list (+ a 2) (+ b 2) (+ c 2)))))
	(mapcar (lambda (f)
		(5am:is (equal (mapcar proc2 xs ys) (funcall f proc2 xs ys)))
		(5am:is (equal (mapcar proc3 xs ys zs) (funcall f proc3 xs ys zs))))
		'(seqops:map-rv seqops:map-iv seqops:map-lpv seqops:map-fv
		seqops:map-uv))))

(5am:test (test-for-each-v :fixture fixc-sequenceops)
	(let* ((xs '(0 1)) (ys '(2 3)) (zs '(4 5))
		(proc2 (lambda (a b) (format t "~a ~a" a b)))
		(proc3 (lambda (a b c) (format t "~a ~a ~a" a b c))))
	(mapcar (lambda (f)
		(5am:is (equal (map nil proc2 xs ys) (funcall f proc2 xs ys)))
		(5am:is (equal (map nil proc3 xs ys zs) (funcall f proc3 xs ys zs))))
		'(seqops:for-each-rv seqops:for-each-iv seqops:for-each-lpv
		seqops:for-each-fv seqops:for-each-uv))))

(5am:test (test-fold-left-v :fixture fixc-sequenceops)
	(let* ((xs '(0 1 2)) (ys '(2 3)) (zs '(3 4))
		(corp2 (lambda (a e1 e2) (+ a e1 e2)))
	...
	(mapcar (lambda (f)
		(5am:is (equal ans2 (funcall f corp2 0 xs ys)))
		(5am:is (equal ans3 (funcall f corp3 0 xs ys zs))))
		'(seqops:fold-left-rv seqops:fold-left-iv seqops:fold-left-lpv))))

(5am:test (test-fold-right-v :fixture fixc-sequenceops)
	(let* ((xs '(0 1 2)) (ys '(2 3)) (zs '(3 4))
		(proc2 (lambda (e1 e2 a) (+ (+ e1 e2) a)))
		...
		(ans3 (reduce (lambda (x-y-z a) (- (+ (car x-y-z) (cadr x-y-z) 
			...
			:from-end t)))
	(mapcar (lambda (f)
		(5am:is (equal ans2 (funcall f proc2 0 xs ys)))
		(5am:is (equal ans3 (funcall f proc3 0 xs ys zs))))
		'(seqops:fold-right-rv seqops:fold-right-iv
		seqops:fold-right-lpv))))

(5am:test (test-append-v :fixture fixc-sequenceops)
	(let* ((xs '(1)) (ys '(2 3)) (zs '(4 5)))
	(mapcar (lambda (f)
		(5am:is (equal (append xs ys) (funcall f xs ys)))
		(5am:is (equal (append xs ys zs) (funcall f xs ys zs))))
		'(seqops:append-rv seqops:append-iv seqops:append-lpv
		seqops:append-fv seqops:append-uv))))

(5am:test (test-zip-v :fixture fixc-sequenceops)
	(let ((xs '(0 1 2)) (ys '(20 30)) (zs '(#\a #\b)) (ws '("Z" "Y")))
	(mapcar (lambda (f)
		(5am:is (equal (mapcar #'list xs ys zs) (funcall f xs ys zs)))
		(5am:is (equal (mapcar #'list xs ys zs ws) (funcall f xs ys zs ws))))
		'(seqops:zip-rv seqops:zip-iv seqops:zip-lpv seqops:zip-fv
		seqops:zip-uv))))
